...
import static com.wms.utils.DateConstant.*;
...
public class DateUtil {
    ...
    public static String getDateCode() {
        Date d = new Date();
        SimpleDateFormat sdf = new SimpleDateFormat(DATE_PATTERN_CODE);
        return sdf.format(d);
    }
    ...
    public static String[][] getYearMonth(Integer year){
        String[][] months = new String[12][2];
        if(year == null) {
            year = Integer.valueOf(format(new Date(),YEAR_PATTERN));
        }
        for(int i = 0 ; i < 12 ; i++) {
    ...
    /**
     * 获得以当前时间,往后几天的时间
     * 比如现在星期1 + add=5 那么返回的就是星期6的现在的时间
     * @param add
     * @return
     */
    public static Date currentAdd(int add){
        Calendar cal = Calendar.getInstance();
        ...
        if(value != 0) { //跨年的
            ...
            for(Integer i : years) { //中间的跨年的
                ...
                for(int j = 0 ; j < midYearMonths.length ; j++) {
                    date[index][0] = midYearMonths[j][0];
                    date[index][1] = midYearMonths[j][1];
                    index++;
                }
            }
            //最后一年的
     ...
     * @return
     */
    public static Date getTodayEnd0() {
        String date =  format(getTodayStart());
        return parse(date, DATE_TIME_PATTERN);
    }
    /**
     * 获得今天开始的时间
    ...
    public static Timestamp oneYear(){
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(getCurrentTimstamp());
        calendar.add(Calendar.YEAR,1);
        return new Timestamp(calendar.getTime().getTime());
    }


    public static Timestamp getExpriedTime(Integer expriedTimeLimit) {
		Calendar instance = Calendar.getInstance();
		instance.setTime(new Date());
		instance.add(Calendar.DAY_OF_YEAR, expriedTimeLimit);
		long timeInMillis = instance.getTimeInMillis();
		return new Timestamp(timeInMillis);
	}


    public static boolean isEx(String exTime){
        Date parse = parse(exTime);
        return getCurrentTimstamp().getTime() > parse.getTime();
    }
//    获取两个Date类型的时间差(精确到秒)
    ...
    public static List<String> initTime(String StartTime,String endTime) throws Exception {
        //创建一个放所有日期的集合
    ...
    public static Timestamp getExpiredTime(){
        return new Timestamp(new Date().getTime() + ( 240 * 60 * 1000));
    }
    public static Timestamp getVerifyTime(){
        return new Timestamp(new Date().getTime() + ( 3 * 60 * 1000));
    }

    public static Timestamp disableTime(){
        return new Timestamp(new Date().getTime() + ( 10 * 60 * 1000));
    }
